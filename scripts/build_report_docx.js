...
body.push(table(["Measure","Value"], [
  ...
  ["Walker-days recorded", money(sum.walker_days)],
...
body.push(note("Boundary distance excludes surveys that are not land boundaries, and the legs walked in to reach a boundary and out again. Who walked on which day, and how far, is a separate document: Managalas Walker Days."));
...
for (const c of clans) {
  ...
  body.push(table(["Walkers","Surveys","Walked (km)","Access legs (km)","Pieces","Not walked (km)","Parcels","Area (ha)","Basis"], [[
  ...
  body.push(table(["Custodian","Tracks","Days","First","Last","Walked (km)"],
    c.walkers.map(w=>[w.custodian || "—", money(w.tracks), money(w.days),
