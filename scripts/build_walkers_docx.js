// Build the walker-days Word document from output/walkers/walkers.json.
//
//   node scripts/build_walkers_docx.js <walkers-dir> <report-json-dir> <out.docx>
...
const data = JSON.parse(fs.readFileSync(path.join(W,'walkers.json'),'utf8'));
const ctx = JSON.parse(fs.readFileSync(path.join(S,'context.json'),'utf8'));
const walkers = data.walkers, days = data.days;
...
const totalKm = walkers.reduce((a,w)=>a+w.km, 0);
const undated = walkers.reduce((a,w)=>a+w.undated_tracks, 0);
...
for (const r of dated) {
  const z = byZone[r.zone] || (byZone[r.zone] = { walkers:new Set(), days:new Set(), walkerDays:0, km:0 });
  z.walkers.add(r.custodian); z.days.add(r.date); z.walkerDays++; z.km += r.km;
}

const body = [];

body.push(new Paragraph({ text:"Managalas Walker Days", heading:HeadingLevel.TITLE }));
...
body.push(table(["Measure","Value"], [
  ["Custodians who walked", money(walkers.length)],
  ["Walker-days recorded", money(dated.length)],
  ...
  ["Average per walker-day", one(totalKm/dated.length)+" km"],
  ["Tracks with no recoverable date", money(undated)],
], [6,3]));
body.push(note("A walker-day is one custodian recording on one day. Two custodians out together on the same day count as two walker-days and one day on the ground."));

body.push(h("By zone", HeadingLevel.HEADING_1));
body.push(table(["Zone","Custodians","Days on the ground","Walker-days","Distance (km)"],
  Object.keys(byZone).sort().map(z=>[z, money(byZone[z].walkers.size),
    money(byZone[z].days.size), money(byZone[z].walkerDays), one(byZone[z].km)]),
  [3,2,3,2,3]));

body.push(h("Every walker", HeadingLevel.HEADING_1));
body.push(p("Sorted by zone, then by name. Distance is measured along the smoothed track, so it is the distance along the ground the receiver recorded, not the distance the walker's legs covered.", {size:21}));
body.push(table(["Custodian","Zone","Clan(s)","Tracks","Days","First","Last","Distance (km)","Per day (km)"],
  walkers.map(w=>[w.custodian, w.zones, w.clans || "—", money(w.tracks), money(w.days),
    w.first || "—", w.last || "—", one(w.km), w.km_per_day===null?"—":one(w.km_per_day)]),
  [4,2,4,2,2,3,3,2,2]));

body.push(new Paragraph({ children:[new PageBreak()] }));
body.push(h("Every walker-day", HeadingLevel.HEADING_1));
body.push(p("One row per custodian per day, in date order within each custodian. This is the full record; the totals above are its sums.", {size:21}));

const byWalker = {};
for (const r of days) (byWalker[r.custodian] || (byWalker[r.custodian] = [])).push(r);
for (const name of Object.keys(byWalker).sort()) {
  const rows = byWalker[name].slice().sort((a,b)=>String(a.date).localeCompare(String(b.date)));
...
body.push(p(`${longDays.length} walker-days record more than ${LONG_DAY_KM} km. In this terrain that is a long way for one day on foot, and there are two ordinary explanations worth ruling out: the receiver left running during travel by vehicle, or a single track recorded across more than one day and saved once. A track is dated by when it was created, so a walk spanning two days is counted on the first.`, {size:21}));
if (longDays.length) {
  body.push(table(["Custodian","Date","Zone","Clan","Tracks","Distance (km)"],
    longDays.map(r=>[r.custodian, r.date, r.zone, r.clan || "—", money(r.tracks), one(r.km)]),
    [4,3,3,4,2,2]));
}
...
if (undated) {
  ...
  body.push(table(["Custodian","Zone","Clan(s)","Undated tracks"],
    walkers.filter(w=>w.undated_tracks).map(w=>[w.custodian, w.zones, w.clans || "—",
...
body.push(...bullets([
  ...
  "A custodian who appears under more than one clan walked for each of them; the days are counted once, under the clan of each track.",
]));

const doc = new Document({
  creator: "MCA Maps",
  title: `Managalas Walker Days v${ctx.version}`,
  ...
  sections: [{
    ...
    footers: { default: new Footer({ children:[ new Paragraph({
      ...
      children:[ new TextRun({ text:`Managalas Walker Days — version ${ctx.version}, ${ctx.date_long}  ·  page `,
